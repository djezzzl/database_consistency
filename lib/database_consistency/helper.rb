...
module DatabaseConsistency
  ...
  module Helper # rubocop:disable Metrics/ModuleLength
    ...
    def normalize_sql(sql)
      # Matches PostgreSQL casts like `column::text` and removes the cast so
      # `column::text <> ''::text` and `column != ''` normalize the same way.
      sql.gsub(/::\w+/, '')
         # Matches a parenthesized bare identifier like `(internal_name)` after
         # cast removal, and unwraps it back to `internal_name`.
         .gsub(/\(([a-z_][\w.]*)\)/i, '\1')
         # Matches boolean literals written as TRUE/FALSE and rewrites them to
         # 1/0 to match adapter-specific SQL generated by Active Record.
         .gsub(/\bTRUE\b/i, '1').gsub(/\bFALSE\b/i, '0')
         # Matches the SQL inequality operator `<>` with optional surrounding
         # whitespace and rewrites it as `!=`.
         .gsub(/\s*<>\s*/, ' != ')
         # Matches `IS NOT NULL` regardless of spacing/case and rewrites it to
         # a stable uppercase form.
         .gsub(/\bIS\s+NOT\s+NULL\b/i, ' IS NOT NULL')
         # Matches `IS NULL` regardless of spacing/case and rewrites it to
         # a stable uppercase form.
         .gsub(/\bIS\s+NULL\b/i, ' IS NULL')
         # Matches PostgreSQL boolean literals serialized as `'t'` / `'f'`
         # inside comparisons and rewrites them to `1` / `0`.
         .gsub(/ = 't'/, ' = 1').gsub(/ = 'f'/, ' = 0')
         # Matches any run of whitespace and collapses it to a single space.
         .gsub(/\s+/, ' ')
         .strip
    end
