  ...
  module Helper # rubocop:disable Metrics/ModuleLength
    ...
    def normalize_sql(sql)
      sql
        ...
        .gsub(/\(([a-z_][\w.]*)\)/i, '\1')
        # `/\bTRUE\b/i` and `/\bFALSE\b/i` normalize boolean literals to `1`
        # / `0` so they match SQL generated by Active Record on some adapters.
        ...
        # `/ = 't'/` and `/ = 'f'/` normalize PostgreSQL boolean literals
        # stored as `'t'` / `'f'` inside comparisons.
        .gsub(/ = 't'/, ' = 1').gsub(/ = 'f'/, ' = 0')
