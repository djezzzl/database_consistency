      ...
      context 'when allow_nil/allow_blank/if/unless is true' do
        ...
        specify do
          expect(compare).to include(
            status: :fail,
            message: 'column field of table entities of model Entity is required but possible null value insert'
          )
        end
      end
    end
  ...
  context 'when database has null: true' do
    ...
    context 'when presence: true' do
      ...
      specify do
        expect(compare).to include(
          status: :fail,
          message: 'column field of table entities of model Entity should be required in the database'
        )
      end
